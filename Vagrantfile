...
Vagrant::configure("2") do |config|
  ...
  machines.each_with_index do |machine, index|
    ...
    config.vm.define machine[:name] do |config|
      ...
      # install redis
      # vagrant up --provision-with terraform to only run this on vagrant up
      config.vm.provision "redis", preserve_order: true, type: "shell", privileged: true, path: "hashicorp/redis.sh"
      ...
      #config.vm.provision "postgresql", run: "never", type: "shell", preserve_order: true, privileged: true, path: "database/postgresql.sh"
	...
	echo -e '\e[38;5;198m'"Traefik UI http://localhost:8181"
	echo -e '\e[38;5;198m'"Documentation UI http://localhost:3333"
